            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-5 space-y-6">
                    <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-6">
                        {/* Bank Selection */}
                        <div>
                            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-3">Select Bank</label>
                            <div className="relative group">
                                <select 
                                    value={bank}
                                    onChange={(e) => setBank(e.target.value)}
                                    className="w-full bg-muted/50 border border-border rounded-xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 font-bold appearance-none cursor-pointer pr-10"
                                >
                            ...
                            <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-indigo-50 dark:bg-indigo-500/5 rounded-lg border border-indigo-100 dark:border-indigo-500/10">
                                <Info size={14} className="text-indigo-500" />
                                <span className="text-[10px] text-indigo-700 dark:text-indigo-300 font-medium uppercase tracking-tight">
                                    Current Rate: {BANK_RATES[bank].general}% (General) | {BANK_RATES[bank].senior}% (Senior)
                                </span>
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-3">
                                <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Investment Amount</label>
                                <span className="text-lg font-black text-indigo-600 dark:text-indigo-400">₹{amount.toLocaleString()}</span>
                            </div>
                            <input 
                                type="range" 
                                min="10000" 
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-2">Years</label>
                                <input 
                                    type="number" 
                                    min="0" 
                                    max="20" 
                            ...
                            <div>
                                <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-2">Months</label>
                                <input 
                                    type="number" 
                                    min="0" 
                                    max="11" 
                        ...
                        <div className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-foreground">Senior Citizen</p>
                                    <p className="text-[10px] text-muted-foreground">Extra ~0.50% interest</p>
                                </div>
                            </div>
                            <button 
                                onClick={() => setIsSenior(!isSenior)}
                                className={`w-12 h-6 rounded-full transition-colors relative ${isSenior ? "bg-indigo-600" : "bg-slate-300 dark:bg-slate-700"}`}
                            >
                                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isSenior ? "left-7" : "left-1"}`} />
                            </button>
                        </div>

                        {/* Compounding */}
                        <div>
                            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-3">Interest Compounding</label>
                    ...
                    <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 flex flex-col h-full justify-between gap-10">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest mb-1">Maturity Value</p>
                                    <h2 className="text-5xl font-black tabular-nums tracking-tighter">₹{results.maturityValue.toLocaleString()}</h2>
                                </div>
                                <div className="p-4 bg-white/20 backdrop-blur-md rounded-2xl">
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-center mb-1">Total Returns</p>
                                    <p className="text-2xl font-black">₹{results.totalInterest.toLocaleString()}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-3 gap-4 border-t border-white/20 pt-8">
                                <div>
                                    <p className="text-indigo-200 text-[10px] font-bold uppercase mb-1">Principal</p>
                                    <p className="font-bold">₹{amount.toLocaleString()}</p>
                                </div>
                                <div>
                                    <p className="text-indigo-200 text-[10px] font-bold uppercase mb-1">Interest Rate</p>
                                    <p className="font-bold">{results.rate}%</p>
                                </div>
                                <div>
                                    <p className="text-indigo-200 text-[10px] font-bold uppercase mb-1">Tenure</p>
                                    <p className="font-bold">{tenureYears}y {tenureMonths}m</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-muted-foreground">Interest Compounding</span>
                        ...
                        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
                            ...
                            <p className="text-xs text-muted-foreground mb-4">How {bank} compares to other major banks for your tenure:</p>
                            <div className="space-y-2">
                                {Object.entries(BANK_RATES).slice(0, 3).map(([name, data]) => (
                                    <div key={name} className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 transition-colors">
                                        ...
                                        <span className="text-xs font-bold text-indigo-600">{isSenior ? data.senior : data.general}%</span>
                                    </div>
                                ))}
                            </div>
                        </div>
